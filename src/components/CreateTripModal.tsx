...
const DEST_OPTIONS: Destination[] = ['Pacific Beach', 'Downtown', 'Grocery', 'Airport'];

const CreateTripModal = () => {
  const { currentUser } = useStore();
  const [open, setOpen] = useState(false);
  const [destination, setDestination] = useState<Destination>('Pacific Beach');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const destCoords = DESTINATIONS.find(d => d.name === destination)?.coordinates || { lat: 32.88, lng: -117.23 };
    api.createTrip({
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {DEST_OPTIONS.map((d) => (
                    <button
                      ...
                      className={`chip ${destination === d ? 'chip-active' : 'chip-inactive'}`}
                    >
                      {d}
                    </button>
                  ))}
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Trunk space, music preferences, vibes..."
                  rows={2}
                  className="w-full px-3 py-2 rounded-lg border border-border bg-card text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring resize-none"
                />
              </div>

              <button
                type="submit"
                className="w-full py-2.5 rounded-lg bg-primary text-primary-foreground font-semibold text-sm hover:brightness-110 transition-all"
              >
                Post Trip 🚀
              </button>
            </form>
